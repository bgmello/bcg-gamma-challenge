...
def lat_from_codigo(codigo):
    '''
    Retorna latitude
    '''
    return coord_from_codigo(codigo)[0] if coord_from_codigo(codigo)[0]!=0 else np.nan

def lon_from_codigo(codigo):
    '''
    Retorna longitude
    '''
    return coord_from_codigo(codigo)[1] if coord_from_codigo(codigo)[1]!=0 else np.nan
...
def clean_time_features(txt):
    '''
    Transforma as colunas de tempo em datetime
    '''
    txt = str(txt)
    if txt[:4]<'2009' or txt[:4]>'2019':
        return np.nan
    try:
