...
class KMedoidsWeighted():
    ...
    def dist_function(self, m, o):
        '''
        Calcula distancia euclidiana utilizando entre cidades
        '''
        return np.sqrt((m[0]-o[0])**2+(m[1]-o[1])**2)
    ...
    def fit(self, X, medoids_fix_idxs, max_itr=1000, verbose=0):
        '''
        Minimiza a função perda achando a melhor configuração de meoides com um conjunto de medoides pre definidos.
        
        X é uma lista de lista da forma (latitude, longitude, população)

        medoids_fix_idxs é o conjunto de medoids pre definidos
        '''
        m = len(X)

        self._generate_dist_matrix(X)
        medoids_idxs = set(np.append(np.array(medoids_fix_idxs), np.random.choice(np.setdiff1d(np.arange(len(X)), medoids_fix_idxs), self.n_clusters - len(medoids_fix_idxs), replace=False)))
